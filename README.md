# AFDM 系统性能研究

基于 MATLAB 的**仿射频分复用（AFDM）**系统综合仿真框架，涵盖嵌入式导频（EP）基线系统、GI-Free 导频分离架构、分数多普勒信道估计，以及 Agile-$c_2$ PAPR 优化。

> **Status**: Active development · MATLAB R2023b+ · Communications Toolbox required

## 研究动机

AFDM 是一种基于 chirp 的多载波波形，通过离散仿射傅里叶变换（DAFT）的前 chirp 参数 $c_1$ 在双色散（时延-多普勒）信道中实现全分集增益。尽管 AFDM 在误码率层面显著优于 OFDM，但传统的**嵌入式导频（EP）**方案将约 30% 的子载波消耗于保护间隔。**GI-Free** 架构消除了保护间隔，将频谱效率从约 70% 提升至 99.6%（$N = 256$ 时），但原始 GI-Free 方案仅适用于整数多普勒场景，且采用经验检测门限，工程适用性有限。

本项目依次解决三个开放性问题：EP 的频谱效率瓶颈、GI-Free 的整数多普勒局限性、以及单高功率导频引入的 PAPR 问题。

## 系统架构

### EP 基线系统

嵌入式导频方案将单个导频置于 DAFT 域索引 0 处，前后各设 $Q$ 个零填充保护子载波。保护间隔从物理上隔绝了数据对导频的干扰（ID2P），使得接收端可以直接通过门限检测提取路径并估计增益。EP 系统在本项目中作为性能基线和频谱效率参照。

### GI-Free 架构（本文工作）

GI-Free 架构保留了 DAFT 域索引 0 处的导频，但完全去除了保护零子载波。其余 $N-1$ 个子载波全部承载数据，频谱效率达到 $(N-1)/N$。由于没有保护间隔，ID2P 直接污染导频响应，因此需要全新的接收机设计。

**接收机流水线（三阶段 turbo 迭代）：**

1. **Phase 1 — 干扰感知粗估计。** OMP 配合 CFAR 自适应停止门限检测路径，LMMSE 均衡产生初始数据估计并消除 ID2P。

2. **Phase 2 — 判决引导精提取。** 利用硬判决符号做参考，通过 Dirichlet 核匹配精炼分数多普勒，PCG 稀疏求解器做均衡。

3. **后判决精炼。** 最终增益重估与噪声功率估计。

## 核心技术贡献

- **分数多普勒支持** — 基于 Dirichlet 核显式建模多普勒泄漏，割线法精搜将估计精度推至 $10^{-4}$ 量级，突破整数多普勒假设下的误差平层。
- **CFAR 自适应检测门限** — 从统计原理出发推导自适应门限，替代固定经验系数，具备随残差功率实时下降的双层自适应能力。
- **Agile-$c_2$ PAPR 优化** — 将逐块 $c_2$ 敏捷搜索集成至完整收发链路，证明了其对信道估计和数据检测的透明性。

## 环境要求

- MATLAB **R2023b** 或更高版本
- **Communications Toolbox**
